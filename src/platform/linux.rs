...
pub fn modify_default_login() -> String {
    ...
    if let Ok(x) = run_cmdsf("ls /usr/share/* | grep ${DESKTOP_SESSION}-xorg.desktop".to_owned()) {
        if x.trim_end().to_string() != "" {
            ...
            run_cmdsf("ls /usr/share/* | grep ${DESKTOP_SESSION:0:-8}.desktop".to_owned())
        {
...
pub fn run_cmdsf(cmds: String) -> ResultType<String> {
